...
> Реализует `ideas/08_panel_knowledge_and_ornith.md`. Статус: 🔧 в работе (2026-07-03).
...
- Модель плотная (не MoE): минимальный квант **IQ3_XXS = 14.87 ГБ** (bartowski) — на грани 16 ГБ.
  Скачиваем в фоне; настраиваем с малым контекстом (KV q8_0, `-c` под остаток VRAM); замеряем
  `bench-model.ps1` (скорость) + `agent-bench.mjs` (ум). Заносим в `researches/02`.
- Ожидание честно: плотная 35B ≈ мало места под контекст → скорее «умный, но тесный» вариант,
  чем дейли-драйвер. Решение (основная/нет) — по цифрам.
