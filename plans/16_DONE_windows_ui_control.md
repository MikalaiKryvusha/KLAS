...
>
> **Статус (2026-07-28, поздний вечер): ✅ ПЛАН ЗАКРЫТ. Фазы 1–4 выполнены, включая контрольный
> прогон фазы 4 НЕ из-под администратора.** Ядро реально управляет UI: Блокнот открыт, `Click`
> отработал, `Type` напечатал текст, ход завершился штатно (`stopReason=stop`, 23 тул-колла, 0 отказов).
> Блокер движка снят (`bugs/05` — виновата была наша схема тула, не билд; движок обновлён до b10167).
> Открытый хвост вынесен отдельным дефектом: `bugs/07` (перемежающееся искажение символа в `Type`).
>
...
>
> **Что оставалось до `DONE` — закрыто 2026-07-28 (поздний вечер):**
> 1. ✅ **Фаза 4 прогнана НЕ из-под администратора.** Прогон 22:00–22:05, лог доказывает права
>    машинно-читаемым признаком: `IsAdmin = False`, `User = KRINIKSPC\krinik`. Итог: 23 тул-колла,
>    **0 отказов**, `stopReason=stop`, 306.9 с; `App`, `Wait`, `Snapshot`, `Screenshot`, `Click`, `Type`
>    отработали с обычными правами. **EXP-0007 опровергнут в части техники:** понизить права изнутри
>    административной сессии МОЖНО — `runas /trustlevel:0x20000` (`explorer.exe` и COM не работали, а
>    это работает). Среду чинить всё равно полезно, но она больше не блокер.
> 2. ✅ **Вопрос одобрений закрыт решением** (в рамках интервью 004 Q4 = «ступенчато, allow-list»):
>    на локальных прогонах защита = **allow-list из 7 тулов + непривилегированный токен + присмотр +
>    запись экрана**; одобрения на действие появляются только в сессиях через гейтвей, и туда же
>    переносится любая АВТОНОМНАЯ работа с UI. Записано в «Решения, принятые без владельца».
> 3. ✅ **Запись экрана стала нормой прогона** (`tools/screen-rec.mjs`): прогон 28.07 писался целиком
>    (645 кадров, `screenrec\2026-07-28T19-00-19_rec`), результат судился кадром в нативном масштабе, а
>    не самоотчётом модели. Сверх нормы: критерий теста теперь **машинный** (см. следующий пункт).
> 4. ✅ **Тест перестал проходить по неверной причине.** Прогон печатал константу `KLAS UIA test`, а
>    Блокнот Windows 11 хранит несохранённую вкладку между запусками — текст от прошлого прогона уже
>    лежал в редакторе, и критерий «текст виден» выполнялся сам собой. Теперь фикстура несёт
>    **уникальный токен прогона**, а вердикт сверяется с ОС (заголовок окна), а не с моделью:
>    `tools/llamacpp-probe.mjs` → `promptUi(token)`, `notepadTitles()`, новый вердикт `FAIL-UI-UNVERIFIED`.
>
...
> 3. **Права понижаются — но не теми средствами, что пробовались сначала.** `explorer.exe` и COM
>    `ShellExecute` дают новый АДМИНСКИЙ процесс (EXP-0007), а вот `runas /trustlevel:0x20000` даёт
>    процесс с `IsAdmin = False` в той же интерактивной сессии — этого достаточно и для UI, и для
>    ограничителя радиуса поражения (UIPI: непривилегированный агент физически не достучится до окон
>    админ-приложений). Рабочая команда:
>    `runas /trustlevel:0x20000 "powershell -NoProfile -ExecutionPolicy Bypass -File <раннер>.ps1"`.
>    Промпт передавать **файлом** (`--message-file`) — иначе кириллицу и кавычки съедает quote-mangling.
>
...
```
> ⚠️ Команда выше — ИСТОРИЧЕСКАЯ (адресация по `label`, константный текст, админские права). Она
> оставлена как след диагностики: на билде b9538 этот прогон падал на грамматике (`bugs/05`).
>
> **Канонический прогон фазы 4 (2026-07-28, действующий)** — непривилегированный, с уникальным
> токеном и машинной сверкой. Промпт живёт в `tools/llamacpp-probe.mjs` (`promptUi(token)`), правки —
> только парой с этим планом:
> ```powershell
> # 1) запись экрана — норма живого UI-прогона (судим кадрами, EXP-0010)
> node tools\screen-rec.mjs start
> # 2) сам прогон — БЕЗ админских прав (runas /trustlevel) и БЕЗ --model (EXP-0005);
> #    промпт передаём файлом: --message-file спасает кириллицу от quote-mangling PowerShell
> runas /trustlevel:0x20000 "powershell -NoProfile -ExecutionPolicy Bypass -File <раннер>.ps1"
> # 3) раннер внутри: openclaw agent --local --session-key uia-nonadmin --json --timeout 300 --message-file <промпт>
> # 4) вердикт: токен прогона обязан найтись в заголовке окна Блокнота (не в самоотчёте модели)
> node tools\screen-rec.mjs stop
> ```
> Готовый однокомандный вариант того же теста: `node tools/llamacpp-probe.mjs <билд> --ui`.
...
- `Type` затирает буфер обмена (≥20 симв.); может ронять символы на нагруженной машине.
  → ⚠️ **Риск РЕАЛИЗОВАЛСЯ и вынесен в `bugs/07`:** модель послала `KLAS UIA test`, на экране осело
  `KLAS IIA test` (подмена символа, перемежающаяся). Значит критерий шага «набрать текст» не может
  быть «тул вернул успех» — только сверка фактически введённого.
- Нужна интерактивная сессия переднего плана; не-elevated (админ-окна недоступны — это ок).

## Решения, принятые без владельца (при закрытии плана 2026-07-28)

1. **Понижение прав через `runas /trustlevel:0x20000`, а не через починку среды.** Владелец просил
   прогнать фазу 4 не из-под администратора; чинить среду (перезапустить VS Code без прав) значило бы
   ждать человека. Выбран технический путь, дающий тот же машинно-проверяемый результат
   (`IsAdmin = False`) — и он оказался надёжнее совета «не запускай от админа», потому что не зависит
   от того, как владелец открыл редактор. **Оговорка:** `trustlevel` даёт УРЕЗАННЫЙ токен (deny-only
   для админ-групп); для нашей задачи это строже, чем обычный пользователь, то есть в безопасную
   сторону. Если владелец предпочитает эталонный «обычный пользователь» — это одна строка через
   разовую задачу планировщика с `/RL LIMITED`.
2. **Одобрения на действие решено НЕ вводить на локальных прогонах.** Механизм `plugin_approval_requested`
   живёт в Control UI гейтвея, а `--local` его не поднимает. Городить свой гейт одобрений — новая
   сущность против принципа минимума. Принято: локально защита = **allow-list 7 тулов +
   непривилегированный токен + присмотр + запись экрана**; любая АВТОНОМНАЯ работа с UI (без человека
   у экрана) — только через гейтвей с одобрениями. Это ровно ступенчатость из интервью 004 Q4.
3. **Критерий фазы 4 ужесточён без спроса.** Тест мог пройти на тексте от прошлого прогона
   (Блокнот хранит несохранённую вкладку) — то есть проходил бы по неверной причине. Введён уникальный
   токен прогона и сверка по состоянию ОС. Считаю это не расширением задачи, а условием того, чтобы
   слово «прогнано» вообще что-то значило.
4. **`bugs/07` заведён, но НЕ чинится в этом плане.** Искажение символа в `Type` — дефект стороннего
   компонента, перемежающийся; лечить его вслепую без изолированного воспроизведения = гадание.
   План закрыт по своей цели (ядро управляет UI), дефект живёт своим документом.
5. **Прогон делался на боевом ПК владельца, а не в VM/Sandbox** — как и все предыдущие: этого требует
   сам смысл фичи (управлять ЕГО рабочим столом). Радиус поражения ограничен allow-list'ом и правами.

**Неулаженных допущений (`PENDING:`) не осталось** — пункты 1–3 подтверждены наблюдением, 4–5
вынесены владельцу этим разделом.
