...
> Реализует `ideas/03_klas_single_unit_self_deploy.md`. Статус: 🔬 ПРОЕКТИРОВАНИЕ (2026-07-03) —
> план готов к ревью, реализация по шагам. Репозиторий — каркас/обёртка/фрейм; всё тяжёлое deploy
> скачивает сам.
...
- [ ] 1. `deploy.manifest.json`: зафиксировать текущие версии (llama.cpp b9538, модель gemma +
        sha256 текущего файла, llama-swap v234, docker-образы) — «снимок живой системы».
- [ ] 2. `deploy.mjs`: каркас (парсинг манифеста, dry-run, предпроверки) + обработчики
        `github-release`, `hf-file`, `url` (скачивание с докачкой + sha256).
- [ ] 3. Обработчики `winget` и `compose`; пост-настройка и финальная проверка харнессом.
- [ ] 4. Полигон-тест на этой машине: «всё уже есть» → ноль скачиваний, зелёный health-check.
