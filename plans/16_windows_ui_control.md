...
>
> **Статус (2026-07-28, вечер): ✅ Фазы 1–3 ВЫПОЛНЕНЫ. Фаза 4 ВЗЯТА — ядро реально управляет UI:
> Блокнот открыт, `Click` отработал, `Type` напечатал текст, ход завершился штатно (`stopReason=stop`).**
> Блокер движка снят (`bugs/05` — виновата была наша схема тула, не билд; движок обновлён до b10167).
> Тег `DONE` пока не ставить: осталась хвостовая работа по безопасности/среде (см. «Что осталось» ниже).
>
...
> - ✅ **Действия ЗАРАБОТАЛИ (вечер 2026-07-28, движок b10167):** `Click` по элементу панели задач и
>   `Type` текста в Блокнот выполнены живьём, ход завершился штатно. Прежний симптом («ход умирает на
>   первом действующем тул-колле, модель отдаёт XML `<tool_call><function=windows__Type>`») был
>   следствием старого движка b9538 — он не парсил XML-формат тул-коллов Qwen3.6.
> - ⚠️ **Поправка к тексту теста:** новый Блокнот Windows 11 **не отдаёт область ввода в UIA** — метки
>   для неё не существует, и требовать `Type` по `label` бессмысленно. Рабочий путь — второй параметр
>   тула: `Type` с `loc: [x, y]` (координаты внутри клиентской области окна). Канонический промпт
>   фазы 4 живёт в `tools/llamacpp-probe.mjs` (константа `PROMPT_UI`) — правки только парой с этим планом.
...
> - ✅ **Блокер снят:** бисекция не потребовалась — причиной 400 была НАША схема тула (`cron`,
>   `maxLength: 65536` против порога llama.cpp 2000), лечение — `compat.unsupportedToolSchemaKeywords`.
>   Движок в бою — **b10167**, откат папкой `llamacpp-b9538`. Разбор: `bugs/05_DONE_llamacpp_toolcall_grammar.md`.
>
> **Что осталось до `DONE` этого плана:**
> 1. Прогнать фазу 4 **не из-под администратора** (сейчас все прогоны шли с админскими правами — это
>    ломает главный ограничитель радиуса поражения, EXP-0007). Лечится средой: VS Code/терминал не от админа.
> 2. Решить вопрос одобрений: при `agent --local` их нет (Control UI не поднят) — либо гонять UI-сессии
>    через гейтвей, либо честно оставить защиту = allow-list 7 тулов + присмотр и записать это решением.
> 3. Прогон под записью экрана как норма (`tools/screen-rec.mjs`) — чтобы результат судился кадрами,
