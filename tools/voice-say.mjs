...
// Охранник «есть ли что произносить» (bugs/06, ревизия 2026-07-31). Исходно требовал КИРИЛЛИЦУ,
// но с 2026-07-30 сайдкар прогоняет текст через text_norm (translit=full): «56» → «пятьдесят
// шесть», «OK» → «Оу-Кей» — и старый охранник глушил ответы, которые тракт давно умеет читать,
// на БОЕВОМ провайдерском пути ядра (симптом bugs/06 воскресал: ответ-число — тишина).
// Отсекаем только текст ВООБЩЕ без букв и цифр — та же семантика, что 'nothing-to-say' демона.
// Отдельный код выхода 2 позволяет вызывающему отличить «нечего произносить» от поломки РТА.
const HAS_SPEAKABLE = /[а-яёА-ЯЁa-zA-Z0-9]/;
if (!HAS_SPEAKABLE.test(text)) {
  console.error(`Нечего произносить: в тексте нет ни букв, ни цифр («${text}») — см. bugs/06.`);
  process.exit(2);
}
