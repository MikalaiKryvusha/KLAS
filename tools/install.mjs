...
async function main() {
  ...
  // ⚠️ ФЛАГ --apply ОБЯЗАТЕЛЕН. Без него anonymize.mjs работает в режиме DRY-RUN (`anonymize.mjs:22`):
  // печатает план и не меняет НИ ОДНОГО файла — мастер рисовал экран успеха над копией, где остались
  // имена, ссылки и origin автора (bugs/23; замер до фикса — 41 файл с личностью).
  // --reinit-git НЕ передаём: стирание истории git необратимо и требует отдельного явного согласия.
  // Судим по РЕЗУЛЬТАТУ, а не по факту запуска: anonymize.mjs сам грепает дерево на остатки личности
  // (PROBES) и выходит кодом 1 при утечках ⇒ провал останавливает мастера ДО экрана успеха — ровно
  // как провал деплоя выше (bugs/21: экран успеха над провалившимся шагом — тот же класс).
  // [TESTED: 2026-08-01 · прогон мастера на свежем клоне в песочнице: 41 файл утечек → 0, валидатор
  //  чист; при подставленном провале обезличивания мастер остановился и экран успеха не напечатал]
  if (answers.anonymous && existsSync(join(ROOT, 'tools', 'anonymize.mjs'))) {
    line(`\n${t('anonymizing')}`);
    if (!runInherit('node', [join(ROOT, 'tools', 'anonymize.mjs'), '--apply'])) {
      line(`\n⛔ ${t('anonymize_failed')}`);
      return;
    }
  }
