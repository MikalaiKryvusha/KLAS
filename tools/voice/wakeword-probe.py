...
def main() -> int:
    ...
    for f in files:
        ...
        # Хвост добиваем нулями до кратности кадра: прежний `range(0, len-1280, 1280)` ВСЕГДА
        # выбрасывал последний полный кадр (off-by-one), а клип короче кадра не подавался вовсе —
        # слово в конце записи получало заниженный score (ревизия 2026-07-31; вердикт 0/10 остаётся
        # в силе — контроль «Alexa.» зажигался тем же циклом).
        pad = (-len(audio)) % 1280
        if pad:
