...
baya 246.2 (246), kseniya 243.7 (245); затем на живых записях — 102.1 / 124.4 / 171.4 / 195.1 Гц
у четырёх разных дикторов. Инструмент точен и на синтезе, и на сжатых архивных mp3.]

> 🧠 **Здесь стояло ЛОЖНОЕ ограничение, и история его стоит того, чтобы её сохранить.**
> Первая партия эталонов дала у всех шести клипов 167–179 Гц. Я решил, что это «удвоение октавы
> на сжатом звуке», и вписал сюда предупреждение не верить регистру. Через час выяснилось, что
> все шесть записей читает ОДИН диктор (Саша Жукова, 171 Гц) — цифры были правильные, а неверным
> был мой вывод. Владелец услышал это за минуту.
> **Урок в чистом виде (PHILOSOPHY: лошади, а не зебры):** когда измерение выдаёт «невозможно
> одинаковый» результат, первая гипотеза — объекты ОДИНАКОВЫ, и только вторая — инструмент врёт.
> Я проверил инструмент контролем, увидел, что он точен, и всё равно обвинил сигнал.
> И отдельно: **ложная граница, записанная в шапку инструмента, опаснее отсутствующей** — она
> выглядит как оплаченное знание и заставила бы следующую сессию выбросить исправный замер.
