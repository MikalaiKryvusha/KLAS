...
#
# ⛔ ЧЕГО ЭТОТ СТЕНД НЕ ИЗМЕРЯЕТ — читать прежде, чем верить его числам.
# Клип имени берётся ИЗ ТОГО ЖЕ КОРПУСА, на котором модель обучалась. Значит стенд отвечает на
# вопрос «узнаёт ли модель то, чему её учили», а НЕ «узнаёт ли она владельца». Пока корпус
# произносит имя так же, как человек, эти вопросы совпадают — и для «Джарвиса» с «Джой» совпадали.
# Для «Ариэль» разошлись: стенд дал 0.985 в тишине и 0.988 под речью, а владелец на живом микрофоне
# сказал «плохо слышит имя, джарвиса и джой вызвать легче» (2026-08-01). Причина нашлась в составе
# корпуса: Piper произносит это имя неустойчиво, отбраковка выбросила 39% клипов НЕРАВНОМЕРНО, и
# доля мужских голосов упала с 70–72% (как у других имён) до 62%.
# ⇒ Высокая оценка здесь НЕ доказывает, что активатор услышит человека. Доказывает это только
# живой микрофон владельца. Второй раз за сессию комната оказалась правее прибора (первый — Джой,
# которую стенд объявил неперебиваемой, а владелец перебивал).
#
...
# ⚠️ ДВА РАЗНЫХ СЛОВА, и путать их нельзя. `WORD` — как персону ЗОВУТ (это видит человек в отчёте),
# `CLIP_WORD` — каким написанием собран КОРПУС. У «Ариэль» они расходятся: Piper это имя не
# выговаривает (29% годных клипов), а написание «Ариель» даёт правильный ЗВУК (58%) — обычная
# практика словарей произношения в TTS (`plans/20`). Стенд ищет клипы по написанию корпуса, иначе
# не найдёт ни одного и упадёт с «нет клипов, где слово заканчивает фразу».
WORD = {"jarvis": "Джарвис", "joy": "Джой", "ariel": "Ариэль"}
CLIP_WORD = {"jarvis": "Джарвис", "joy": "Джой", "ariel": "Ариель"}
...
def pick_name_clip(slug):
    ...
    with open(man, encoding="utf-8") as f:
        m = json.load(f)
    w = CLIP_WORD[slug]   # написание КОРПУСА, а не имя персоны (см. комментарий у CLIP_WORD)
