...
// ⚠️ Атаки увеличены после замечания «резкий фронт, режет ухо». Прежние 20–45 мс щёлкали заметно.
// Коридор выбран так, чтобы НЕ вернуться к набуханию, которое владелец забраковал раньше: свелл у
// него был 200+ мс, здесь потолок 120 мс — звук всё ещё «ударяют и он тает», но вход не режет.
const WARM_SPECS = {
  // 1. Базовый: вход 60 мс.
  'bell': { lo: 293.66, hi: 440, sec: 0.75, attack: 0.06, partials: [1, 0.25], soft: 1.1, hiAmp: 0.3 },
  // 2. Длиннее хвост — тает дольше, звучит спокойнее.
  'bell-long': { lo: 293.66, hi: 440, sec: 1.15, attack: 0.06, partials: [1, 0.25], soft: 1.7, hiAmp: 0.3 },
  // 3. Самый мягкий вход из допустимых — 120 мс. Дальше начинается тот самый свелл.
  'bell-soft': { lo: 293.66, hi: 440, sec: 0.9, attack: 0.12, partials: [1, 0.2], soft: 1.4, hiAmp: 0.28 },
  // 4. Чистые синусы без гармоник — самый гладкий тембр, вход 90 мс.
  'bell-pure': { lo: 293.66, hi: 440, sec: 0.95, attack: 0.09, partials: [1], soft: 1.5, hiAmp: 0.3 },
  // 5. Выбран владельцем на закрытие. Ниже (ля большой октавы + ми), вход 80 мс.
  'bell-deep': { lo: 220, hi: 329.63, sec: 1.05, attack: 0.08, partials: [1, 0.22], soft: 1.6, hiAmp: 0.32 },
};
...
// ⚠️ ТИШИНА В НАЧАЛЕ ФАЙЛА — НЕ УКРАШЕНИЕ. Владелец: «у сигналов очень резкий фронт, слышно, режет
// ухо, когда он запускается». Резкость даёт не огибающая: проигрыватель поднимается отдельным
// процессом и ОТКРЫВАЕТ звуковое устройство, а само открытие слышно щелчком — сколько ни сглаживай
// тон, щелчок будет раньше него. Лечение известное: дать устройству открыться в паузе, а тон начать
// после. 120 мс хватает с запасом и на слух не задерживает сигнал.
const LEAD_SILENCE = 0.12;

function toWav(rawSamples) {
  const samples = concat([silence(LEAD_SILENCE), rawSamples]);
