...
# ⚠️ ОДИН ИСТОЧНИК ПРАВДЫ ДЛЯ НОРМАЛИЗАЦИИ (найдено 2026-07-30).
# Слой нормализации был «вынесен» отсюда в `text_norm.py` — но тракт на него НЕ ПЕРЕКЛЮЧИЛИ,
# и в файле осталась вторая копия. Полтора дня правки произношения (дроби, код, транслитерация
# латиницы, снятое двоеточие) жили только в модуле и до ЖИВОГО ассистента не доезжали.
# Классическая цена дубля: вынести реализацию мало — надо СНЯТЬ старую, иначе «вынес» означает
# «сделал вторую».
from text_norm import normalize  # noqa: E402

# Режим латиницы для БОЕВОГО рта. Silero — русская модель; латиницу она отдаёт ВТОРОЙ модели,
# то есть другому диктору, и владелец услышал это как «разные голоса… НЕПРИЕМЛИМО» (bugs/11).
# Его решение способа — канон 2026-07-29: «транслитерация кириллицей, второй способ самый хороший».
# При `full` латиницы не остаётся вовсе ⇒ английская модель не вызывается ⇒ ОДИН диктор на реплику.
# ⚠️ Честная граница, названная владельцем 2026-07-30: «транслитерация — это вынужденный костыль».
# Она и есть костыль — но именно ДЛЯ ОДНОЯЗЫЧНОГО движка. Настоящее лечение — мультиязычная модель
# (Qwen3-TTS, CosyVoice 3), где английское произносится английским тем же голосом. Пока рантайм
# одноязычный, костыль честнее двух дикторов в одной фразе.
TRANSLIT_MODE = "full"
...
# --- Русские числа/единицы/markdown ЗДЕСЬ БОЛЬШЕ НЕ ЖИВУТ ---
# Они переехали в `text_norm.py` (импорт `normalize` наверху файла). Здесь стояла ВТОРАЯ копия —
# и именно она сделала так, что полтора дня правок произношения не доезжали до живого ассистента.
# Не возвращать: одна реализация на один факт (DRY), правки вносятся в `text_norm.py`.
# Английский разворот чисел ниже НЕ дубль — его в `text_norm` нет вовсе (тот разворачивает
# числа только по-русски), и он нужен, пока TRANSLIT_MODE может быть не 'full'.
...
# (список единиц и снятие markdown переехали в `text_norm.py` — см. пометку выше)
...
def synth_segments(text, voice_ru, voice_en, torch):
    ...
    # ОБЩИЙ слой `text_norm`, а не местная копия: markdown → код → единицы → латиница → числа.
    # Числа он разворачивает СРАЗУ для всего текста, поэтому ниже по-русски их разворачивать уже
    # нечего — вызов оставлен только для английских отрезков (они появляются, если TRANSLIT_MODE
    # не 'full').
    text = normalize(text, translit=TRANSLIT_MODE)
    for lang, chunk in split_by_script(text):
        ...
        # Русские числа уже развёрнуты общим слоем выше; английские — здесь, потому что
        # `text_norm` разворачивает числа ТОЛЬКО по-русски, а в латинском отрезке они звучали бы дико.
        if lang == "en":
            chunk = digits_to_en_words(chunk)
