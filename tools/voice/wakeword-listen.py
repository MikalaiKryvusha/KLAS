    ...
    for audio in source:
        ...
        while not cmds.empty():
            ...
            if c.get("cmd") == "speaking":
                ...
                # ровно в тот миг, когда слушатель уже завёл захват: событие идёт по трубе, и к
                # моменту, когда приходит «я звучу», захват успел начаться с pending=False. Дальше
                # автомат принимал колокольчик за начало речи и «заканчивал фразу» на нём.
                # Поэтому объявление, пришедшее ПОСРЕДИ захвата, взводит его заново — но только
                # пока человек ещё не заговорил: обрывать настоящую речь нельзя.
                if speaking and capture is not None and not capture["ep"].talking:
                    capture["pending"] = True
                    capture["frames"] = []
                    quiet_wait = 0
                    # Счётчики автомата тоже назад: иначе кадры, съеденные сигналом, укоротили бы
                    # обещанное владельцу окно ожидания речи.
                    capture["ep"].frames = 0
                    capture["ep"].silence = 0
