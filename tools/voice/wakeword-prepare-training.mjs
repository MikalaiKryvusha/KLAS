...
// ⭐ ПОТОЛОК ДЛИТЕЛЬНОСТИ ПОЛОЖИТЕЛЬНЫХ КЛИПОВ — самая важная ручка этого файла.
//
// Опыт 2026-08-01 (scratchpad/align-probe, числа в плане 19): модель РЕЗКО чувствительна к тому,
// где слово стоит внутри двухсекундного окна.
//     «Джарвис.» словом в КОНЦЕ окна  → 0.983
//     тот же звук словом в НАЧАЛЕ     → 0.001
// Так и задумано: `create_fixed_size_clip` (data.py) выравнивает клип по ПРАВОМУ краю окна с
// джиттером до 200 мс, то есть учит узнавать слово в момент, когда оно ТОЛЬКО ЧТО закончилось.
//
// Отсюда прямое следствие, которое я проглядел при проектировании корпуса: клип ДЛИННЕЕ окна
// обрезается случайной половиной (`data.py:676`, первые 2 с либо последние 2 с), и слово из длинной
// фразы уезжает не туда. Такой пример не выучивается и не узнаётся — «Джарвис, какая погода?» даёт
// 0.043. Отсюда двугорбое распределение: медиана 0.906 при пятом перцентиле 0.001, то есть треть
// положительных не узнаётся вовсе, и полнота застревает на 64%.
//
// Я добавил длинные обрамления, РАССУЖДАЯ про «устойчивость к выравниванию», вместо того чтобы
// прочитать, как архитектура раскладывает данные. Домысел вместо наблюдения (`PHILOSOPHY.md`).
//
// Лечение не требует пересинтеза: короткие клипы в корпусе уже есть, их надо просто отобрать.
