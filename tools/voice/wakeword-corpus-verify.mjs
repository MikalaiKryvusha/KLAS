...
for (const c of manifest.clips) {
  ...
  if (positive) {
    ...
    neg++;
    // ⚠️ У НЕГАТИВОВ ВОПРОС ДРУГОЙ, И СУДИТЬ ИХ НАДО ИНАЧЕ.
    // У положительных мы ЗНАЕМ, что слово произносилось, и спрашиваем «пережило ли оно синтез» —
    // там уместна фонетическая близость. У негативов слова в тексте нет ПО ПОСТРОЕНИЮ, и вопрос
    // ровно один: не прозвучало ли оно всё-таки. Это ТОЧНОЕ совпадение.
    // Замер 2026-07-31 показал, почему разница не косметическая: нечёткая сверка забраковала
    // «Дарвин описал происхождение видов» — а «дарвин» отстоит от «джарвис» всего на 2 правки.
    // Это не отравленный негатив, это САМЫЙ ЦЕННЫЙ трудный негатив набора: ровно на таких словах
    // детектор и будет ошибаться, если их не показать. Выбросить его — обеднить обучение.
    const exact = words(text).includes(EXPECT);
    if (exact) negHit.push({ ...c, text, distance, word });
    else {
      if (Number.isFinite(distance) && distance <= tolerance(EXPECT) + 1) hardNeg.push({ ...c, text, distance, word });
...
if (negHit.length) {
  console.log(`\n❌ Отрицательные, где слово прозвучало ТОЧНО (${negHit.length}) — их надо убрать из текстов:`);
  for (const m of negHit.slice(0, 8)) console.log(`  ${m.file}  «${m.text}»`);
}
if (hardNeg.length) {
...
if (negHit.length) problems.push(`слово прозвучало ТОЧНО в ${negHit.length} отрицательных — они отравляют обучение`);
