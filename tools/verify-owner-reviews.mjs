...
function livePilot() {
  ...
  for (const rel of docs) {
    ...
    // 🔑 Шапка обязана называть, КТО спрашивает и КОГДА: у владельца несколько проектов, и открытая
    // страница не должна опознаваться по содержанию (правка владельца 2026-08-02).
    if (!/Спрашивает ИИ-агент <b>[^<]+<\/b> · \d{2}\.\d{2}\.\d{4}/.test(head)) {
      noAsker++;
      console.log(`         ⛔ ${rel}: шапка не называет проект и время`);
    }
    // Пилюли-счётчики обязаны отражать РЕАЛЬНОЕ число вопросов, а не быть украшением.
    const o = iv.questions.filter((q) => !q.answered).length;
    const d = iv.questions.length - o;
    const wantPills = iv.questions.length
      ? head.includes(`ждут вас: ${o}`) && head.includes(`отвечено: ${d}`)
      : /вопросов нет/.test(head);
    if (!wantPills) {
      badPills++;
      console.log(`         ⛔ ${rel}: пилюли не сходятся с разбором (открыто ${o}, отвечено ${d})`);
    }
  ...
  ok('шапка называет ПРОЕКТ и ВРЕМЯ («Спрашивает ИИ-агент …»)', noAsker === 0, `без шапки: ${noAsker}`);
  ok('пилюли-счётчики сходятся с разбором', badPills === 0, `расхождений: ${badPills}`);
