...
**Статус:** ✅ закрыто (2026-07-04) — подтверждено владельцем. Пульт открывается локально
(`http://localhost/`), ссылка чата ведёт куда надо, статус/логи llama-swap UI живые, а поиск по
Kiwix из чатов Open WebUI работает (идея 11 — тул `search_local_knowledge`, проверено: модель
вызывает, отвечает по локальной Википедии).
