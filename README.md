...
Репозиторий — каркас: тяжёлое (движок, модель, docker-образы) скачивается при развёртывании.

```powershell
git clone https://github.com/MikalaiKryvusha/KLAS.git F:\KLAS
node F:\KLAS\tools\deploy.mjs          # репетиция: печатает план
node F:\KLAS\tools\deploy.mjs --apply  # реальное развёртывание
```

Требования: Windows 11, NVIDIA GPU (16 GB VRAM рекомендовано), Node ≥20, git, winget;
docker — опционально (база знаний и дашборд). Статус: ⚗️ проверено на живой системе
(идемпотентность); полный тест с нуля — впереди.

**Анонимная установка** (обезличенная копия, без следов автора и связи с origin):

```powershell
git clone https://github.com/MikalaiKryvusha/KLAS.git F:\KLAS
node F:\KLAS\tools\anonymize.mjs                      # репетиция: что будет вычищено
node F:\KLAS\tools\anonymize.mjs --apply --reinit-git # выжечь автора/origin + стереть git-историю
node F:\KLAS\tools\deploy.mjs --apply                 # затем обычное развёртывание
```

После анонимизации имя автора и ссылки на его GitHub удалены из всех файлов, git-история стёрта,
проект больше не обновляется из origin — установить авторство по проекту невозможно.
