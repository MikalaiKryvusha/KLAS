...
| Голос (в работе) | [Silero](https://github.com/snakers4/silero-models) TTS (резидентный) + [GigaAM-v3](https://github.com/salute-developers/GigaAM) STT через sherpa-onnx — полностью офлайн, на CPU | KLAS говорит и слышит; ход занимает ~8 с от вопроса до ответа. Английские слова произносятся **транслитерацией кириллицей** — чтобы диктор оставался ОДИН на всю реплику (две модели давали смену голоса посреди фразы). Клоны персон Jarvis/Joi — в подборе |
